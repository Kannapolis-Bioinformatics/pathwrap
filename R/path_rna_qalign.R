...
run_qAlign <- function(corenum, endness, sampleFile, genomeFile, geneAnnotation, ref.dir, cacheDir, aligner) {
    ...
    if (!is.na(ref.dir)) {
        if (length(list.files(ref.dir, ".Rhisat2$", full.names = TRUE)) != 1) {
            ...
            write.table(sampleFiletmp, sep = "\t", col.names = TRUE, row.names = FALSE, file = sampleFiletmp_name)
    ...
    g <- ggplot(data, aes(fill = typesofdata, y = value, x = stringr::str_remove_all(genomeofsamples, ":genome"))) +
