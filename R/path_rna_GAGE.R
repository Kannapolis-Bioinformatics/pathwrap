...
run_gage <- function(gsets, work.dir, same.dir,compare ,fc_matrix , entity) {
    ...
    gage.out<-cbind(gage.res$greater[, c(2,5)], pms/2, p.val=pgs.gene, 
                    q.val=qgs.gene)
...
#paste0(anla_type, ".gage.res.gs.heatmap.pdf"),fsep = .Platform$file.sep))
        ...
        message("No gene set selected in 1d-test, select the top 3 instead!")
        ...
        if(sum(gsel)>0) gpath.ids<-rownames(gage.out)[seq_len(min(sum(gsel),3))]
