...
run_gage <- function(gsets, work.dir, same.dir,compare ,cpd_data, entity,
                    gene_data ,cpd.idtype , gene_id_type ="entrez") {
#referenced from codes for pathview web available at https://pathview.uncc.edu/
    print("This is running the gage should not be deleted")
    qcut<-0.01
    ...
    datatype , ".tsv")), sep="\t", col.names=NA,row.names = TRUE    , quote = FALSE)
    ...
    if (same.dir == FALSE){
        plotpathways(kegg.dir = work.dir, entity, gpath_ids[seq_len(6)] , 
                    cpd_data = cpd_data, gene_data =gene_data ,
                    cpd_id_type = cpd.idtype, gene_id_type ="entrez")}
