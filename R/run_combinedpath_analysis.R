...
#' @param q_cutoff threshold for pathway selection
#' @import gage
#' @return pathids important pathways of interest
#' @export
run_combinedpath_analysis <- function(outdir, gene_data, entity, gcompare,gage.out.cpd_res,q_cutoff ){
...
# }
    ...
    #kegg.gs$kg.sets[kegg.gs$met.idx]
    #kegg.gsets(entity)$kg.sets
    ...
    sig.i<- gage.out[,"q.val"]<q_cutoff & !is.na(gage.out[,"q.val"])
   ...
   pathids <- combinedpath_analysis(gpath_ids, cpath_ids,gsets, pgs.gene,
    pgs.cpd, work.dir, gage.out, gage.out.cpd,q_cutoff)
   return(pathids)
}
