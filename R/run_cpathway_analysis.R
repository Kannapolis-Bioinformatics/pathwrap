...
run_cpathway<- function( cdatapath,cpd_id_type= "KEGG COMPOUND accession",csamp,
    ...
    if (grepl(".csv$", x = cdatapath)){
        cpd_data <- read.csv(cdatapath, header = T, row.names = 1)
    } else{
        cpd_data<- read.table(file = cdatapath,sep="\t",header= TRUE,row.names = 1)
    }
    
    if (cpd_id_type!="KEGG COMPOUND accession"){# | cpd_id_type != "KEGG"){
        cpd_idmap<-cpd2kegg(in.ids=rownames(cpd_data),in.type=toupper(cpd_id_type))
    ...
    print("this is what I am checkiing")
    print(rownames(cpd_data))
    csets <- loadcsets()
